...
def home():
        return jsonify({"welcome message":"Welcome to my API!! Added a custom secret to the JWT to increase security.",
                        "endpoints":{"login [POST]":"/login?username=&password=",
                        "create user [POST]":"/create?username=&password=",
                        "admin [GET]":"/admin",
                        "source [GET]":"/source"}}), 200
...
def login():
    ...
    if 'username' in data and 'password' in data:
        ...
        if login_check(username, password) == True:
            # Added a static token at ./static_token with a value of "statictoken123"
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=2533)
